...
type Result =
  ...
  | { status: 'error'; statusCode: number | undefined; data: unknown };
...
async function runConcurrencyTest() {
  ...
  try {
    ...
    if (!availableSeat) { console.log('No available seats found.'); return; }
  ...
  const requests = Array.from({ length: CONCURRENCY_LEVEL }, () =>
    axios.post(`http://localhost:5000/api/shows/${showId}/seats/${seatId}/hold`, {}, {
      headers: { Authorization: `Bearer ${token}` }
    })
      .then(res => ({ status: 'success', data: res.data }) as Result)
      .catch(err => ({ status: 'error', statusCode: err.response?.status as number | undefined, data: err.response?.data }) as Result)
  );

  const results = await Promise.all(requests);

  const successes = results.filter(r => r.status === 'success');
  const conflicts = results.filter(r => r.status === 'error' && (r as Extract<Result, { status: 'error' }>).statusCode === 409);
  const others = results.filter(r => r.status === 'error' && (r as Extract<Result, { status: 'error' }>).statusCode !== 409);
